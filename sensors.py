...
if is_on_raspberry_pi():
    ...
    class Sensors(SensorsABC):
        ...
        def __init__(self, dump_communication=False):
            self._pressure_mux = I2CMux(
                tetra_constants.PRESSURE_SENSOR_MUX_ADDRESS,
                dump_communication=dump_communication)
            self._pressure_sensors = []
            for i in range(tetra_constants.NUMBER_OF_PRESSURE_SENSORS):
                ...
                self._pressure_sensors[i].set_sampling(
                    ...
                    pressure_sampling_rate=(
                        tetra_constants.PRESSURE_SAMPLING_RATE),
                    temperature_oversample=(
                        tetra_constants.TEMPERATURE_OVERSAMPLING),
                    temperature_sampling_rate=(
                        tetra_constants.TEMPERATURE_SAMPLING_RATE)
                )
                self._pressure_sensors[i].set_op_mode(
                    PressureSensor.OpMode.command)

            self._flow_mux = I2CMux(tetra_constants.FLOW_SENSOR_MUX_ADDRESS,
                                    dump_communication=dump_communication)
            self._flow_sensors = []
            for i in range(tetra_constants.NUMBER_OF_SENSIRION_SENSORS):
            ...
            for i in range(tetra_constants.NUMBER_OF_MASS_AIRFLOW_SENSORS):
                pass
        ...
        def connected_sensors(self):
            def sensors_available_on_port(i):
                ...
                if self._pressure_sensors[i].is_present():
                    port_i.append(tetra_constants.PRESSURE_SENSOR)
                if i < tetra_constants.NUMBER_OF_SENSIRION_SENSORS:
                    self._flow_mux.select_channel(i)
                    if self._flow_sensors[i].is_present():
                        port_i.append(tetra_constants.SENSIRION_SENSOR)
                return tuple(port_i)

            return tuple(sensors_available_on_port(i)
                         for i in range(tetra_constants.MAX_SENSOR_COUNT))

        def tubes_with_enough_sensors(self):
            tubes = []
            sensors = self.connected_sensors()
            for i in range(len(sensors)):
                if (tetra_constants.PRESSURE_SENSOR in sensors[i]
                    and (tetra_constants.SENSIRION_SENSOR in sensors[i]
                         or (tetra_constants.MASS_AIRFLOW_SENSOR
                             in sensors[i]))):
                    tubes.append(i)
            if len(tubes) != tetra_constants.NUMBER_OF_PATIENTS:
        ...
        def calibration_pressure_sensor_connected(self):
            if (tetra_constants.PRESSURE_SENSOR in
                self.connected_sensors()[
                    tetra_constants.CALIBRATION_PRESSURE_SENSOR_INDEX]):
                return True
            else:
                return False
        ...
        def poll(self):
            ...
            def sensor_data_on_port(i):
                ...
                if tetra_constants.SENSIRION_SENSOR in sensors[i]:
                    data.append(self._flow_sensors[i].flow())
                if tetra_constants.MASS_AIRFLOW_SENSOR in sensors[i]:
                    pass
                return data
    ...
    class Sensors(SensorsABC):
        ...
        def connected_sensors(self):
            try:
                if (os.environ[tetra_constants.SENSOR_QUANTITY]
                        == tetra_constants.NOT_ENOUGH_SENSORS):
                    return tuple([(tetra_constants.PRESSURE_SENSOR,)]
                                 + [(tetra_constants.PRESSURE_SENSOR,
                                     tetra_constants.SENSIRION_SENSOR)
                                    for _ in range(tetra_constants.
                                                   NUMBER_OF_PATIENTS-1)])

                elif (os.environ[tetra_constants.SENSOR_QUANTITY]
                      == tetra_constants.TOO_MANY_SENSORS):
                    return tuple([(tetra_constants.PRESSURE_SENSOR,
                                   tetra_constants.SENSIRION_SENSOR,
                                   tetra_constants.MASS_AIRFLOW_SENSOR)]
                                 + [(tetra_constants.PRESSURE_SENSOR,
                                     tetra_constants.SENSIRION_SENSOR)
                                    for _ in range(tetra_constants.
                                                   NUMBER_OF_PATIENTS-1)
